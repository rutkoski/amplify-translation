    ...
    @foreach($locales as $locale)
        ...
        <td>
            <a href="#edit" class="editable status-{{ $t ? $t->status : 0 }} 
                locale-{{ $locale }}
                {{ $t && substr($t->value, -2) == strtoupper($currentLocale) && config('amplify-translation.highlight_locale_marked') ? ' editable-locale-empty' : '' }}" 
                data-locale="{{ $locale }}" 
                data-name="{{ $locale . "|" . $parent_key . $key }}" 
                id="username" 
                data-type="textarea" 
                data-pk="{{ $t ? $t->id : 0 }}" 
                data-url="{{ route('amplify-translation.edit', $group) }}" 
                data-title="{{ trans('amplify-translation::panel.rows.edit') }}">{{ $t ? htmlentities($t->value, ENT_QUOTES, 'UTF-8', false) : '' }}</a>
        </td>
    @endforeach
    @if($deleteEnabled)
        <td>
            <a class="delete-key btn btn-danger btn-sm" 
            data-confirm="{{ trans('amplify-translation::panel.rows.confirm-delete', ['key' => $key]) }}" 
            nohref 
            data-url="{{ route('amplify-translation.delete', [$parent_key . $key, $group]) }}" 
            style="cursor: pointer" 
            title="{{ trans('amplify-translation::panel.actions.delete') }}"><i class="glyphicon glyphicon-trash"></i></a>
        </td>
    @endif
</tr>
@endif